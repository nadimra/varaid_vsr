...
def footballvids():
    ...
    if mode == 'HR':
        img_folder = '/vol/bitbucket/nr421/Zooming-Slow-Mo-CVPR-2020/datasets/FootballVids/HR'
        lmdb_save_path = '../data_scripts/footballvids_train_HR.lmdb'
        txt_file = '/vol/bitbucket/nr421/Zooming-Slow-Mo-CVPR-2020/datasets/FootballVids/footballvids_trainlist.txt'
        H_dst, W_dst = 560, 1118
    elif mode == 'LR':
        img_folder = '/vol/bitbucket/nr421/Zooming-Slow-Mo-CVPR-2020/datasets/FootballVids/LR'
        lmdb_save_path = '../data_scripts/footballvids_train_LR.lmdb'
        txt_file = '/vol/bitbucket/nr421/Zooming-Slow-Mo-CVPR-2020/datasets/FootballVids/footballvids_trainlist.txt'
        H_dst, W_dst = 140, 279
    n_thread = 40
    ########################################################
    ...
    for line in train_l:
        ...
        sub_sub_folder = line.split('/')[2]

        file_l = glob.glob(osp.join(img_folder, folder, sub_folder,sub_sub_folder) + '/*')
        all_img_list.extend(file_l)
        for j in range(5):
            keys.append('{}_{}_{}_{}'.format(folder, sub_folder,sub_sub_folder, j + 1))
    ...
    meta_info = {}
    if mode == 'HR':
        meta_info['name'] = 'FootballVids_train_HR'
    elif mode == 'LR':
        meta_info['name'] = 'FootballVids_train_LR'
    meta_info['resolution'] = '{}_{}_{}'.format(3, H_dst, W_dst)
    key_set = []
    for key in keys:
        print(key)
        a, b, c,_ = key.split('_')
        key_set.append('{}_{}_{}'.format(a, b,c))
    meta_info['keys'] = key_set
    pickle.dump(meta_info, open(osp.join(lmdb_save_path, 'FootballVids_train_keys.pkl'), "wb"))
