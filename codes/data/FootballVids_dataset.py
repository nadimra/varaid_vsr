...
class FootballVidsDataset(data.Dataset):
    '''
    Reading the training FootballVids dataset
    key example: match001/clip01/0001/im1.png
    GT: Ground-Truth;
    ...
    def __init__(self, opt):
        super(FootballVidsDataset, self).__init__()
        self.opt = opt
    ...
    def _read_img_mc_BGR(self, path, name_a, name_b,name_c):
        ''' Read BGR channels separately and then combine for 1M limits in cluster'''
        img_B = self._read_img_mc(
            osp.join(path + '_B', name_a, name_b, name_c + '.png'))
        img_G = self._read_img_mc(
            osp.join(path + '_G', name_a, name_b, name_c + '.png'))
        img_R = self._read_img_mc(
            osp.join(path + '_R', name_a, name_b, name_c + '.png'))
    ...
    def __getitem__(self, index):
        ...
        name_a, name_b, name_c = key.split('_')

        center_frame_idx = random.randint(2, 2)  # 2<= index <=6
        ...
        if self.opt['border_mode']:
            ...
            if center_frame_idx + interval * (N_frames - 1) > 5:
                direction = 0
            ...
            while (center_frame_idx + self.half_N_frames * interval >
                   5) or (center_frame_idx - self.half_N_frames * interval < 1):
                center_frame_idx = random.randint(2, 4)
            # get the neighbor list
            neighbor_list = list(
        ...
        for v in neighbor_list:
            if self.data_type == 'mc':
                img_GT = self._read_img_mc_BGR(
                    self.GT_root, name_a, name_b, name_c, '{}.png'.format(v))
                img_GT = img_GT.astype(np.float32) / 255.
            elif self.data_type == 'lmdb':
                img_GT = util.read_img(
                    self.GT_env, key + '_{}'.format(v), (3, 560, 1118))
            else:
                img_GT = util.read_img(None, osp.join(
                    self.GT_root, name_a, name_b, name_c, 'im{}.png'.format(v)))
            img_GT_l.append(img_GT)

       # get LQ images
        LQ_size_tuple = (3,140, 279) if self.LR_input else (3,560, 1118)
        img_LQ_l = []
        for v in self.LQ_frames_list:
            if self.data_type == 'mc':
                img_LQ = self._read_img_mc(
                    osp.join(self.LQ_root, name_a, name_b,name_c, '/{}.png'.format(v)))
                ...
                img_LQ = util.read_img(None,
                                       osp.join(self.LQ_root, name_a, name_b,name_c, 'im{}.png'.format(v)))
